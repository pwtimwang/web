<?php
define('UPPSITE_MAX_TITLE_LENGTH', 30);
...
function uppsite_process_post($with_content = false) {
	...
	if (function_exists('has_post_thumbnail') && has_post_thumbnail()) {
		$thumb_url = uppsite_extract_src_url(get_the_post_thumbnail());
	}
	$ret = array(
		'id' => get_the_ID(),
		'permalink' => get_permalink(),
    ...
    if ($with_content) {
    	...
    	$orgLen = mb_strlen($ret['title']);
	    if ($orgLen > $maxChar) {
	    	preg_match('/(.{0,' . $maxChar . '}.*?)\b/u', $ret['title'], $matches);
	    	$ret['title'] = rtrim($matches[1]);
	    	$ret['title'] .= (strlen($ret['title']) == $orgLen) ? "" : "..."; 	    }
    }
    return $ret;
}
